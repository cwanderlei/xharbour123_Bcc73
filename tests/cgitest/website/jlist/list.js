...
function listSetImages( cTrue, cFalse ) {
 imgTrue  = cTrue  ;
 imgFalse = cFalse ;
}
...
function _writeList() {
  ...
  for(var i = 0; i < this.types.length; i++) { 
    ...
    if(this.types[i] == "list") {
      str += "<TD WIDTH=15 NOWRAP VALIGN=MIDDLE><A TARGET='_self' HREF=\"javascript:expand("+this.lists[i].id+");\"><IMG BORDER=0 SRC=\"minus.jpg\" NAME=\"_img"+this.lists[i].id+"\"></A></TD>";
      //str += "<TD WIDTH=15 NOWRAP VALIGN=MIDDLE><A TARGET='_self' HREF=\"javascript:expand("+this.lists[i].id+");\"><IMG BORDER=0 HEIGHT=16 SRC=\"'+imgTrue+'\" NAME=\"_img"+this.lists[i].id+"\"></A></TD>";
      _pid++;
    } else if(this.space)
  ...
  for(var i = 0; i < this.types.length; i++) { 
    ...
    if(this.types[i] == "list") {
      if(this.lists[i].visible) {
        ...
        if(isNav4) layer.document.images[0].src = "minus.jpg";
        else eval('document.images._img'+this.lists[i].id+'.src ="minus.jpg"');
      } else {
        if(isNav4) layer.document.images[0].src = "plus.jpg";
        else eval('document.images._img'+this.lists[i].id+'.src ="plus.jpg"');
      }
