...
METHOD Query( cQuery ) CLASS TFbServer
   RETURN TFBQuery():New( ::db, RemoveSpaces( cQuery ), ::dialect )

METHOD TableExists( cTable ) CLASS TFbServer

   LOCAL cQuery, lReturn := .F., qry

   cQuery := "SELECT COUNT(*) QTDE FROM RDB$RELATIONS WHERE RDB$FLAGS=1 AND RDB$RELATION_NAME= '" + Upper( cTable ) + "'"

   qry := FBQuery( ::db, RemoveSpaces( cQuery ), ::dialect )

   IF ISARRAY( qry )
      DO WHILE FBFetch( qry ) == 0
         if alltrim(FBGetData( qry, 1 )) != '0'
            lReturn := .t.
			endif
      ENDDO
...
METHOD ListTables() CLASS TFbServer
   ...
   cQuery += 'where rdb$system_flag = 0;'

   qry := FBQuery( ::db, RemoveSpaces( cQuery ), ::dialect )

   IF ISARRAY( qry )
      DO WHILE FBFetch( qry ) == 0
         AAdd( result, alltrim(FBGetData( qry, 1 )) )
      ENDDO
...
METHOD Delete( oRow, cWhere ) CLASS TFbServer
   ...
   IF ! ISNUMERIC( ::db ) .AND. Len( aTables ) == 1
   ...
   IF ! ISNUMERIC( ::db ) .AND. Len( aTables ) == 1
      ...
      FOR i := 1 TO oRow:FCount()
         IF oRow:Changed( i )
            ...
            cQuery += alltrim(oRow:FieldName( i )) + ","
         ENDIF
      NEXT
...
METHOD Update( oRow, cWhere ) CLASS TFbServer
   ...
   IF ! ISNUMERIC( ::db ) .AND. Len( aTables ) == 1
      ...
      FOR i := 1 TO oRow:FCount()
         IF oRow:Changed( i )
            cQuery += alltrim(oRow:FieldName( i )) + " = " + DataToSql( oRow:FieldGet( i ) ) + ","
         ENDIF
      NEXT
...
STATIC FUNCTION KeyField( aTables, db, dialect )
   ...
   IF Len( aTables ) == 1
      ...
      cQuery += "   b.rdb$relation_name = '" + StrTran(DataToSql( cTable ),'"')+"'"
      cQuery += " order by                                    "
